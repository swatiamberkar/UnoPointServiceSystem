...
public class RM_TicketSummaryPage extends BaseCode {

	public static List<WebElement> webElement;

	JavascriptExecutor javascript = (JavascriptExecutor) driver;

	RM_TicketSummaryPage technician_TicketSummaryPage;
	RM_ViewIncidentPage technician_ViewIncidentPage;

	// Initializing Page Object
	public RM_TicketSummaryPage() {
	...
	public void eTATimeOfAssignedButNoETATab() {
		...
		Integer countOfRows = listOfRows.size();
		for (int i = 0; i < countOfRows; i++) {
			List<WebElement> listOfETATime = driver
					.findElements(By
							.cssSelector("table[class='fht-table fht-table-init']>tbody>tr>td:nth-of-type(28)"));
	...
	public void countOfETAExpiredAndNoATATab() {
		...
		int actualTotalCount = Integer.parseInt(totalCount);
		System.out.println("number of Total Ticket: " + actualTotalCount);
		Assert.assertEquals("countOfRows", "actualTotalCount",
				"Count is not match");
	...
	public void totalCountOfETAExpiredAndNoATATab() {
		...
		String textOfCount = driver
				.findElement(
						By.cssSelector("a[onclick='submitopenetaviolated();']>span[class='badge']"))
				.getText();
		int count = Integer.parseInt(textOfCount);
		System.out.println("number of Request open " + count);
		...
		((JavascriptExecutor) driver).executeScript(
				"arguments[0].scrollIntoView(true);", driver.findElement(By
						.cssSelector("a[onclick='submitopenetaviolated();']")));
	...
	public void aTAOfWorkInProgressTab() throws Exception {
		...
		technician_ViewIncidentPage = new RM_ViewIncidentPage();
		setMyView();
	...
	public void countOfRequestsPausedTab() {
		...
		System.out.println("number of Total Ticket: " + actualTotalCount);
		Assert.assertEquals(listOfRows.size(), Integer.parseInt(totalCount),
				"Count is not match");
	...
	public void totalCountOfRequestsCancelTab() {
		...
		String textOfCount = driver
				.findElement(
						By.cssSelector("a[onclick='submitCancel();']>span[class='badge']"))
				.getText();
		int count = Integer.parseInt(textOfCount);
		System.out.println("number of Request " + count);
		...
		((JavascriptExecutor) driver).executeScript(
				"arguments[0].scrollIntoView(true);", driver.findElement(By
						.cssSelector("a[onclick='submitCancel();']")));
		((JavascriptExecutor) driver)
				.executeScript(
						"arguments[0].setAttribute('style', 'background: yellow; border: 2px solid red;');",
						driver.findElement(By
	...
	public void sumOfAllRequestTab() {
		String textOfCountOfOpenRequests = driver
				.findElement(
						By.cssSelector("a[onclick='submitOpen();']>span[class='badge']"))
				.getText();
		...
		String textOfCountOfPausedRequests = driver
				.findElement(
						By.cssSelector("a[onclick='submitPaused();']>span[class='badge']"))
				.getText();
		int countOfPausedRequests = Integer
				.parseInt(textOfCountOfPausedRequests);
		System.out.println("number of Request Paused " + countOfPausedRequests);

		String textOfCountOfCancelRequests = driver
				.findElement(
						By.cssSelector("a[onclick='submitCancel();']>span[class='badge']"))
				.getText();
		int countOfCancelRequests = Integer
				.parseInt(textOfCountOfCancelRequests);
		System.out.println("number of Request Cancel " + countOfCancelRequests);

		String textOfCountOfTechnicianClosedRequests = driver
				.findElement(
						By.cssSelector("a[onclick='submitTechClosed();']>span[class='badge']"))
				.getText();
		int countOfTechnicianClosedRequests = Integer
				.parseInt(textOfCountOfTechnicianClosedRequests);
		System.out.println("number of Request open "
				+ countOfTechnicianClosedRequests);

		String textOfCountOfClosedRequests = driver
				.findElement(
						By.cssSelector("a[onclick='submitClosed();']>span[class='badge']"))
				.getText();
		int countOfClosedRequests = Integer
				.parseInt(textOfCountOfClosedRequests);
		System.out.println("number of Request open " + countOfClosedRequests);

		int Sum = countOfOpenRequests + countOfPausedRequests
				+ countOfCancelRequests + countOfTechnicianClosedRequests
				+ countOfClosedRequests;
