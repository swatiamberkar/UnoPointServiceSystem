...
public class RM_ViewSubTicketPage extends BaseCode {
	...
	// lblel Date in Incident Date Time
...
//View Sub Ticket Table (Parent Table)
	...
	List<WebElement> lstNameOfColumnInTable;
...
// Child Table
	...
	// Select list of Id in Table
	@FindBy(xpath = ".//*[@id='firstcol']/table/tbody/tr/td/span/a")
	...
	// Select list of Id in Table
	@FindBy(xpath = ".//*[@id='table_div']/table/tbody/tr/td[11]")
...
//ETA And Technician
	...
	List<WebElement> dtpETACalendar;
...
// ATA Tab //
		...
		// Machine Availblle in ATA Details Page
		@FindBy(css = ".modal-content>div>div>table>tbody>tr:nth-of-type(6)>td>select")
		WebElement ddlMachineAvailblleInATADetailsPage;
		...
		WebElement incidentTypeDetails;
 ...
 // final close
    	...
    	WebElement lblCloseDateCalendar;

    	// select close date calendar button
    	@FindBy(css = "div[id='incidentTypeDetails']>table>tbody>tr:nth-of-type(2)>td>a")
    	WebElement symbolCloseDateCalendar;
...
// Pagination //
	...
	{
		 SoftAssert softAssert = new SoftAssert();
		int sizeOfId = lstIDInParentTable.size();
		...
		for (int i = 0; i < sizeOfId - 1; i++) {
			...
			((JavascriptExecutor) driver).executeScript(
					...
					lstIDInParentTable.get(i));
			
			((JavascriptExecutor) driver).executeScript(
					"arguments[0].setAttribute('style', 'background: yellow; border: 2px solid red;');", 
					lstIDInParentTable.get(i));

			String getIDInTable = lstIDInParentTable.get(i).getText();
			...
			String getSpareRequestID2 = lstIDInParentTable.get(i + 1)
					.getText();
	...
	public void displayColumnHeader() throws Exception {
		...
		} else 
		{
			...
			int sizeOfTableColumn = lstNameOfColumnInTable.size();
			System.out.println("Total Table Column: " + sizeOfTableColumn);

			for (int i = 1; i < sizeOfPopupColumn; ) {
				Thread.sleep(1000);
				...
				String tableColumnName = lstNameOfColumnInTable.get(1)
						.getText();
	...
	// Hyper link of ID should have present
		public void checkHyperlinkOfID() throws Exception 
		{
			...
			Integer countOfRow = lstIDInParentTable.size();
			for (int i = 0; i < countOfRow; i++) {
				...
				String ID = lstIDInParentTable.get(i).getText();
	...
	public void checkHyperlinkOfCustomer() 
	{
	       SoftAssert softAsert = new SoftAssert();
			...
			Integer countOfRow = lstCustomerInParentTable.size();
			for (int i = 0; i < countOfRow; i++) 
			{
				...
				String customerValue = lstCustomerInParentTable.get(i).getText();
				...
				{
					lstCustomerInParentTable.get(i).click();
	...
	{
		String incidentHistory = driver.getWindowHandle();
		Integer countOfRow = lstAssetIdInParentTable.size();
		for (int i = 0; i < countOfRow; i++) 
		{
			...
			String assetIdValue = lstAssetIdInParentTable.get(i).getText();
			...
			if (customerIsPresent = true) {
				lstAssetIdInParentTable.get(i).click();
				...
				Assert.assertEquals(splitName, "Incident History of Customer  ");
				...
				Assert.assertTrue(customerIsPresent,
						"customer hyperlink is not enable");
	...
	public void checkHiddenMessageOfAddress() throws Exception
	{
		...
		loop: for (int p = 0; p < sizePagination; p++) 
		{
			...
			Integer countOfRow = lstIncorrectAddressInParentTable.size();
			...
			for (int i = 0; i < countOfRow; i++) {
				...
				((JavascriptExecutor) driver).executeScript(
						"arguments[0].scrollIntoView(true);",
						lstIncorrectAddressInParentTable.get(i));
				...
				String flag = lstIncorrectAddressInParentTable.get(i)
						.getAttribute("alt");
				System.out.println("Flag:" + flag);

				if (flag.equalsIgnoreCase("green")) {
					String flagMassage = lstIncorrectAddressInParentTable
	...
	for (int p = 0; p < sizePagination; p++) {
		...
		try {
			...
			if (sizePagination - 1 == p) {
				softAssert.assertTrue(false, " Ticket is not Availblle");
				System.out.println("Ticket is not Availblle");
			}
			e.printStackTrace();
		}
	}
	...
	public RM_ViewSubTicketTest verifyTicketIdTextBox(String ticketID)
			throws Exception 
	{
		SoftAssert softassert = new SoftAssert();
		Thread.sleep(2000);
		...
		try {
			...
			if (rowSize == 1) {
				...
				softassert.assertEquals(textOfTotalCount,
						"  Total Ticket Count: 1", "Count is not match");

			} else {
				String textOfTotalCount = lblCount.getText();
				System.out.println("textOfTotalCount : " + textOfTotalCount);
				softassert.assertEquals(textOfTotalCount,
						"  Total Ticket Count: 0", "Count is not match");
			}

		}

		softassert.assertAll();
		return new RM_ViewSubTicketTest();

	}

	// verify Ticket ID with multiple data
	public RM_ViewSubTicketTest verifyAssetIdTextBox(String assetID)
			throws Exception 
	{
		SoftAssert softassert = new SoftAssert();
		Thread.sleep(2000);
		...
		txtAssetIdAssetId.clear();

		// send the alphabetical value in Ticket ID text box
		txtAssetIdAssetId.sendKeys(assetID);
	...
	public void matchStatusOfParentTicket() 
	{
		SoftAssert softAssert = new SoftAssert();
		int sizeOfId = lstIDInParentTable.size();
		...
		for (int i = 0; i < sizeOfId; i++) {
			...
			String statusValueOfId = lstStatusInParentTable.get(i).getText();
	...
	public void matchSubTicketIdWithParentTicketId() 
	{
		...
		for (int i = 0; i < sizeOfId; ) {

			String statusValueOfId = lstIDInParentTable.get(i).getText();
			System.out.println("status Value Of Id:-" + statusValueOfId);
			...
			lstIDInParentTable.get(i).click();
			...
			for (int j = 0; j < sizeOfIdInSubTicket-1; j++) {
				...
				assertTrue(isSubTicketIdContainParentTicketId);
				...
				assertTrue(iDIsDescending, "All IDs is in descending order");
	...
	public void clickOnSubTicket() throws Exception 
	{
		...
		int sizePagination = ddlPagination.size();
		System.out.println(sizePagination);
		loop1:for (int p = 0; p < sizePagination; p++) {
		...
		loop: for (int i = 0; i < sizeOfId; i++) 
		{
			...
			String idValue = lstIDInParentTable.get(i).getText();
	...
	public void checkTheSubTicketFormWindow() throws Exception
	{
		...
		assertEquals(nameOfSubTicketForm, "Sub Ticket Form", "Ticket Form Name Is Different");
...
//ETA And Technician
	...
	public void punchETAandTchnician() throws Exception 
	{
		...
		lblETACalendar.click();

		// select Date in Calendar

		for (WebElement day : dtpETACalendar) {
	...
	public void displayNotificationWithoutETAInATATab() throws Exception {
		...
		// Get Notification
		// Thread.sleep(5000);
		String notificationText = lblNotification.getText();
	...
	public void checkATADateAndTimeWithIncidentDateAndTime() throws Exception
	{
		...
		for (WebElement day : lblDayCalendarInATADetailsPage) 
		{
			...
			if (dayInCalendar.equalsIgnoreCase(previousDay)) {
				Thread.sleep(1000);
				day.click();
				String getDateValue = day.getText();
	...
	public void countTotalPauseTimeInPauseTab()
	{
		SoftAssert softAssert = new SoftAssert();
...
//Technician close
	...
	public void compareCloseDateWithTechnicianDateTime() throws Exception 
	{
		...
		if (technicianDate.compareTo(closeDate) >= 0) {
			...
			// get the message from that alert box
			String notificationMessage = lblNotification.getText();
...
//If user click on Update details button in Final close details popup without enter Final close time  then notification should display as 'Final close date/time should be greater than Tech close date/time.'
	...
	public void checkNotificationWithoutSelectingTime() throws Exception 
	{
		...
		for (WebElement day : day_CalendarInTechnicianClosePage) {
			...
			if (dayInCalendar.equalsIgnoreCase(nextDate)) {
				Thread.sleep(1000);
				day.click();
				String getDateValue = day.getText();
				System.out.println("Select the date:-" + getDateValue);
	...
	public void closeSubTicket() throws  Exception 
	{
		...
		for (WebElement day : day_CalendarInTechnicianClosePage) {
			...
			if (dayInCalendar.equalsIgnoreCase(hrs)) {
				Thread.sleep(1000);
				day.click();
				String getDateValue = day.getText();
				System.out.println("Select the date:-" + getDateValue);
		...
		public void checkParentTicketStatusAfterCloseAllSubTicket() throws Exception 
		{
			...
			loop: for (int i = 0; i < sizeOfId; i++) {
			    ...
			    idValue = lstIDInParentTable.get(i).getText();
				System.out.println("idValue:-"+idValue);
				...
				secondLoop:for (int k = 0; k < sizeOfIdInSubTicket; k++)
				{
					...
					if (!statusInSubTicketValue.equals("Closed")) 
					{
						...
						rm_ViewSubTicketPage = new RM_ViewSubTicketPage();
