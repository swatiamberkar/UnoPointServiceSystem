...
public class SD_DailyActivityPage extends BaseCode {
	...
	public void varifyHeadingOfAddDailyActivityPopup() throws Exception {
		...
		Assert.assertEquals(heading, "Add Daily Activity",
	...
	public void checkNotificationWithSelectionAndVarifyDataInAddDailyActivityFrom()
		...
		try {
			...
			// Verify Location
			Assert.assertEquals(selectedLocation, locationInTable,
					"Location is not Same.");

			// Verify Incident ID
			Assert.assertEquals(selectedIncidentID, incidentIDInTable,
			...
			Assert.assertEquals(selectedActivityHeader, activityHeaderInTable,
					"Activity Header is not Same.");

			// Verify Activity
			Assert.assertEquals(activityText, activityInTable,
					"Activity is not Same.");

			// Verify Remark
			Assert.assertEquals(remarkText, remarkInTable,
					"Remark is not Same.");
	...
	public void checkNotificationWithoutSelectionInAddDailyActivityPopup()
		...
		Assert.assertEquals(alertWithoutDate, "Please Select Date.",
		...
		// Verify Alert message
		Assert.assertEquals(alertWithoutTime, "Please Select Time.",
				"Alert in Add Daily Activity popup is not Same");
		// Click on OK button of Alert
		alertTime.accept();
		...
		// Verify Alert message
		Assert.assertEquals(alertWithoutlocation, "Please Select Location.",
				"Alert in Add Daily Activity popup is not Same");
		// Click on OK button of Alert
		...
		// Verify Alert message
		Assert.assertEquals(alertWithoutIncidentID,
		...
		// Verify Alert message
		Assert.assertEquals(alertWithoutCustomerName,
		...
		// Verify Alert message
		Assert.assertEquals(alertWithoutHeader,
		...
		// Verify Alert message
		Assert.assertEquals(alertWithoutActivity, "Please Enter Activity.",
				"Alert in Add Daily Activity popup is not Same");
		// Click on OK button of Alert
		...
		// Verify Alert message
		Assert.assertEquals(alertWithoutRemark,
		...
		for (; incidentID < sizeOfFilterByIncidentID; incidentID++) {
			...
			if (sizeOfIncidentID > 0) {
				...
				for (int j = 0; j < sizeOfIncidentID; j++) {
					...
					// Verify Incident ID
					Assert.assertEquals(incidentID, incidentIDOptions,
							"Incident ID is not Same");
				...
				dailyActivityPageOfSdRole = new SD_DailyActivityPage();
	...
	public void checkDataFilterByCustomerName() throws Exception {
		...
		for (; customerName < sizeOfFilterByCustomerName; customerName++) {
			// Get Customer Name
			...
			if (sizeOfCustomerName > 0) {
				...
				for (int j = 0; j < sizeOfCustomerName; j++) {
					...
					// Verify Customer Name
					Assert.assertEquals(customerName, customerNameOptions,
		...
		for (; activityHeader < sizeOfFilterByActivityHeader; activityHeader++) {
			...
			if (sizeOfActivityHeader > 0) {
				...
				for (int j = 0; j < sizeOfActivityHeader; j++) {
					...
					// Verify Activity Header
					Assert.assertEquals(activityHeader, activityHeaderOptions,
							"Activity Header is not Same");
	...
	public void checkDaysAccordingDates() throws Exception {
		...
		// Verify Days Count
		Assert.assertEquals(day, uniqueDate, "Days Count is not Same.");
	...
	public void checkTimeAccordingTAT() throws Exception {
		...
		// Verify Hours
		Assert.assertEquals(totalHrs, hours, "Hours are not Same.");
		// Verify Minutes
		Assert.assertEquals(totalMin, minute, "Minutes are not Same.");
	...
	public void updateDailyActivity() throws Exception {
		...
		Assert.assertEquals(heading, "Update Daily Activity",
		...
		// Verify Remark
		Assert.assertEquals(remarkAfterUpdate, remark, "Remark is not Same.");
		...
		// Verify Notification
		Assert.assertEquals(notificationText, "Activity Updated Successfully.",
	...
	public void deleteDailyActivity() throws Exception {
		...
		// Verify Alert message
		Assert.assertEquals(alertOfConfirmation,
		...
		// Verify Notification
		Assert.assertEquals(notificationText, "Activity Deleted Successfully.",
				"Notification is not Match");
	...
	public void clickOnDeleteSymbolInDailyActivity() throws Exception {
		...
		System.out.println("Size After Delete: " + sizeAfterDelete);
		// Verify Size
		Assert.assertEquals(sizeBeforeDelete, sizeAfterDelete,
				"Entry get deleted.");
