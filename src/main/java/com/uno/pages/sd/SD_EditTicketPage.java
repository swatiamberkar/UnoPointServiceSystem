...
public class SD_EditTicketPage extends BaseCode {
	...
	List<WebElement> lstTechnicianInTable;
	...
	WebElement btnEditInIncidentDetailsTab;
	...
	WebElement ddlTicketTypeInTicketDetailsPopup;
	...
	// Drop Down Mode in Ticket Details Pop up
	@FindBy(css = ".modal-content>div>table>tbody>tr>td:nth-of-type(1)>table>tbody>tr:nth-of-type(4)>td>select")
	WebElement ddlModeInTicketDetailsPopup;

	// Drop Down Level in Ticket Details Pop up
	@FindBy(css = ".modal-content>div>table>tbody>tr>td:nth-of-type(1)>table>tbody>tr:nth-of-type(6)>td>select")
	WebElement ddlLevelInTicketDetailsPopup;

	// Drop Down CTS in Ticket Details Pop up
	@FindBy(css = ".modal-content>div>table>tbody>tr>td:nth-of-type(1)>table>tbody>tr:nth-of-type(8)>td>select")
	WebElement ddlCTSInTicketDetailsPopup;

	// Drop Down Impact in Ticket Details Pop up
	@FindBy(css = ".modal-content>div>table>tbody>tr>td:nth-of-type(2)>table>tbody>tr:nth-of-type(2)>td>select")
	WebElement ddlImpactInTicketDetailsPopup;

	// Drop Down Urgency in Ticket Details Pop up
	@FindBy(css = ".modal-content>div>table>tbody>tr>td:nth-of-type(2)>table>tbody>tr:nth-of-type(4)>td>select")
	WebElement ddlUrgencyInTicketDetailsPopup;

	// Drop Down Trouble Category in Ticket Details Pop up
	@FindBy(css = ".modal-content>div>table>tbody>tr>td:nth-of-type(2)>table>tbody>tr:nth-of-type(6)>td>select")
	WebElement ddlTroubleCategoryInTicketDetailsPopup;

	// Drop Down Trouble Sub-Category in Ticket Details Pop up
	@FindBy(css = ".modal-content>div>table>tbody>tr>td:nth-of-type(2)>table>tbody>tr:nth-of-type(8)>td>select")
	WebElement ddlTroubleSubCategoryInTicketDetailsPopup;
	...
	@FindBy(xpath = ".//*[@id='sapredetailsdiv']/table/tbody/tr/td[11]")
	...
	WebElement dropdown_PartCategoryInRequiredSpareTab;
	...
	List<WebElement> list_ActivityStatus;
	
	// select date from activities Tab
	@FindBy(xpath = ".//*[@id='Activities']/div[1]/table/tbody/tr/td[4]")
	List<WebElement> list_ActivityDetails;
	...
	public void verifyRequestorOrIncidentTab() throws Exception {
		...
		loop: for (int p = 0; p < sizePagination; p++) {
			...
			for (int i = 0; i < status.size(); i++) {
				...
				String flag = lstFlagOfIncorrectCustomerAddressInTable.get(i)
						.getAttribute("alt");
	...
	public void checkUpdatedTicketDetails() throws Exception {
		...
		/*
		 * // Select Level Select levelDropDown = new
		 * Select(dropdown_LevelInTicketDetailsPopup); // Value of Level String
		 * levelValue = "Level1"; // Select Value of Level
		...
		String troubleCategoryValue = "Demo";
		 ...
		 * label_LevelInIncidentDetailsTab.getText(); String level =
	...
	public void overrideCustomerAddress() throws Exception {
		...
		String addressValue = "Marol Naka, Andheri";
		Thread.sleep(1000);
		Alert alert = driver.switchTo().alert();
		Thread.sleep(2000);
		alert.sendKeys("Marol Naka, Andheri");
		...
		public void selectTicketForPunchETA() throws Exception {
			...
			loop: for (int p = 0; p < sizePagination; p++) {
				...
				if (sizePagination - 1 == p) {
					...
					ticketFromOfViewTicketPage = new SD_EditTicketPage();
...
*/
		...
		try {
		 ...
		 Thread.sleep(1000);
			...
			.assertEquals(technicianAlertMessage,
					"DO You Want To Assign Other Calls To The Same Engineer For This Asset.", "Alert Message is not Same.");
		} catch (Exception e) {
			...
			softAssert
			.assertEquals(technicianAlertMessage1,
					"DO You Want To Assign Other Calls To The Same Engineer For This Asset.", "Alert Message is not Same.");
	...
	public void matchTechnicianName() throws Exception {
		...
		/*
		 * // Thread.sleep(1000); Select state; state = new
		 * Select(driver.findElement(By.name("m_strState")));
		 * state.selectByValue("Maharashtra");
		 * System.out.println("Maharashtra");
		 * 
		 * // click on go button of state driver.findElement(
...
*/
		...
		// in
		// technician text box
		String selectedName = label_TechnicianInETAAndTechnicianTab.getText();
		System.out.println("selected technician name is:-" + selectedName);
		String splitName = selectedName.split(" ")[2];
		System.out.println("splitName: "+splitName);
		// Match that both value is same or not
		highLight(label_TechnicianInETAAndTechnicianTab);
		softAssert.assertEquals(splitName,split,"Technician Name is not Same.");
...
*/
	...
	public void matchETADateTime() throws Exception {
		...
		String etadate = textbox_ETADateInETADetailsPage.getAttribute("value");
		...
		// Format Time
				...
				String appointmentDate = textBox_AppointmentDate.getAttribute("value");
				...
				scrollDown(textBox_AppointmentTime);
		...
		// ETA text box
		String selectedDate = label_ETADateInETAAndTechnicianTab.getText();
		System.out.println("selected Date is:-" + selectedDate);
		String splitDate = selectedDate.split(" ")[3];
		System.out.println("splitName: "+splitDate);
		
		String selectedTime = label_ETATimeInETAAndTechnicianTab.getText();
		...
		String appointTime = appointmentTimeBeforeDataEnter.split(" ")[2];
		System.out.println("splitName: "+appointTime);

		
		highLight(label_AppointmentDate);
		// Match that both ETA Date value is same or not
		softAssert.assertEquals( appointDate,appointmentDate, "Appointment Date is not same");
		System.out.println("The Date is same");
		Thread.sleep(1000);
		
		highLight(label_AppointmentTime);
		// Match that both ETA Time Minute value is same or not
		softAssert.assertEquals( appointTime, apppointmentTimetext,"Appointment Time is not same");
		System.out.println("The Time is same");
		Thread.sleep(1000);
		
		highLight(label_ETADateInETAAndTechnicianTab);
		// Match that both ETA Date value is same or not
				softAssert.assertEquals(etadate, splitDate, "ETA Date is not same");
				System.out.println("The Date is same");
				Thread.sleep(1000);

				highLight(label_ETATimeInETAAndTechnicianTab);
				// Match that both ETA Time Minute value is same or not
		...
		// Match that both ETA Date value is same or not
		softAssert.assertTrue(DateIsPresent, "The Date is not same");
		System.out.println("The Date is same");

		// Match that both ETA Time Minute value is same or not
		softAssert.assertTrue(selectedTimeIsPresent, "The Time is not same");
		System.out.println("The Time is same");

		// Match that both ETA Time Second value is same or not
		softAssert.assertTrue(SelectedTimeSecondPresent,
				"The Second Time is not same");
		System.out.println("The Time is same");*/
	...
	public void displayNotificationWithoutETAInATATab() throws Exception {
		...
		loop: for (int p = 0; p < sizePagination; p++) {
			...
			int size = lstETADateInTable.size();
			System.out.println(size);
			for (int i = 0; i < size; i++) {
	...
	// In Total pause time(minutes) column, number of minutes should display
		...
		public void countTotalPauseTimeInPauseTab() throws Exception {
			...
			// alert box
			textbox_SearchId.sendKeys("6634");
			...
			for (int i = 0; i < size; i++) {
				...
				if (ETA.equals("") && ATA.equals("")) {
					...
					if (ATADate.equalsIgnoreCase("ATA Date :")
						...
						/*
						 * if(notificationText.contains("SMS Sent Successfully"))
						 * { btnCloseNotification.click(); } Thread.sleep(5000);
						 * String notificationValue = notification.getText();
						 * System.out.println(notificationValue);
						 */
						Thread.sleep(1000);
	...
	public void matchPartname() throws Exception {
		...
		for (int j = 0; j < size1; j++) {
...

			{
				...
				scrollDown(list_PartNameInSpareDetail.get(j));
				...
				// Spares Window
				scrollDown(list_PartNameInSpareDetail.get(j));
				highLight(list_PartNameInSpareDetail.get(j));
	...
	public void selectNonConsumableProduct() throws Exception {
		...
		// or
		// not
		highLight(textBox_ProductClassificationInAddSpare);
		scrollDown(textBox_ProductClassificationInReplaceSpare);
		highLight(textBox_ProductClassificationInReplaceSpare);
		softAssert.assertEquals(productClassificationValue,
				productClassificationValueInReplaceSapre);

		// Match both the Part Description value same or not
		highLight(textBox_PartDescriptionInAddSpare);
		highLight(textBox_PartDescriptionInReplaceSpare);
		highLight(textBox_PartDescriptionInReplaceSpare);
	...
	public void checkTransactionType() throws Exception {
		...
		scrollDown(list_TransactionTypeOfSapre.get(getTotalSize - 1));
		highLight(list_TransactionTypeOfSapre.get(getTotalSize - 1));
	...
	public void checkWithFaultySpare() throws Exception {
		...
		// to Replaced
		highLight(list_TransactionTypeOfSapre.get(getTotalSize - 1));
		highLight(list_StatusOfSapre.get(getTotalSize - 1));
	...
	public void matchTechnicianCloseMessageWithDevice() throws Exception {
		...
		highLight(message_SerialNumber);
	...
	public void checkNotificationWithSelectionInSpareTab() throws Exception {
		...
		scrollDown(label_PartNameInSpareTab.get(size1));
	...
	public void checkUpdatedSpareRemarkInSpareTab() throws Exception {
		...
		// Thread.sleep(1000);
		int size = label_SpareRemarkInSpareTab.size();
		System.out.println(size);
		scrollDown(label_SpareRemarkInSpareTab.get(size - 1));
		String SpareRemark = label_SpareRemarkInSpareTab.get(size - 1)
				.getText();
		System.out.println(SpareRemark);
		highLight(label_SpareRemarkInSpareTab.get(size - 1));
		softAssert
	...
	public void checkAlertWithoutSelection() throws Exception {
		...
		// select the Activity
				Select activity;
				activity = new Select(dropdown_Activity);
				...
				// not
				softAssert.assertEquals(alertMessage1, "Please select Activity Status.");
				System.out.println("Both the message is equal");
				...
				// verify that alert message is Please select Activity.or
				// not
				softAssert.assertEquals(alertMessage2, "Please enter Activity Details.");
				System.out.println("Both the message is equal");

				driver.findElement(By.xpath(".//*[@id='boxActivity']/div/table/tbody/tr[1]/th[3]/img")).click();
	...
	public void matchActivityDetailsAndDateAndTime() throws Exception {
		...
		Thread.sleep(1000);
				...
				// select the Activity
						...
						textbox_ActivityDetail.sendKeys("Testing");
		...
		String activityName = list_Activity.get(timeSize - 1).getText();
		System.out.println(activityName);
		
		String activityStatusTxt = list_ActivityStatus.get(timeSize - 1).getText();
		System.out.println(activityStatusTxt);
		
		String activityDetailsTxt = list_ActivityDetails.get(timeSize - 1).getText();
		System.out.println(activityDetailsTxt);
		
		highLight(list_Activity.get(timeSize - 1));
		softAssert.assertEquals(activityName, "Additional Activity", "Activity is not Same");
		highLight(list_ActivityStatus.get(timeSize - 1));
		softAssert.assertEquals(activityStatusTxt, "Complete", "Activity Status is not Same");
		highLight(list_ActivityDetails.get(timeSize - 1));
		softAssert.assertEquals(activityDetailsTxt, "Testing", "Activity Details is not Same");
		highLight(list_DateOfActivities.get(dateSize - 1));
		softAssert.assertEquals(dateValue, SystemDateValue, "Date is not Same");
		highLight(list_TimeOfActivities.get(timeSize - 1));
